...
async def create_booking(data: schemas.CreateBookingRequestSchemas, session: AsyncSession = Depends(get_session)):
...
async def booking_list(data: schemas.ListBookingRequestSchemas = Body(), session: AsyncSession = Depends(get_session)):
...
async def booking_delete(booking_id: int, session: AsyncSession = Depends(get_session)):
